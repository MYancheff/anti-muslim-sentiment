...
for(i in seq(from = 2, to =  157, by = 3)){
...
ReliState <- t(ReliState) %>%
   data.frame() %>%
    rownames_to_column(var = "State") %>%
  slice(-1) %>%
  rename(relig = X1, muslim = X2) %>%
  spread(key = relig, value= muslim) %>%
  mutate(ReliState = as.character(ReliState))
...
```{r, load data}
State_crime <- read_csv("/Users/emilyclark/Desktop/Junior/States/Crimes_state.csv")
ACS <-read_csv("/Users/emilyclark/Desktop/Junior/States/ACS_State.csv")
